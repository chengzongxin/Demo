...
+ (BOOL)hookMethodForClass:(Class)class originalMethod:(NSString*)originalMethodName isInstanceMethod:(BOOL)isInstanceMethod hookedBlock:(RuntimeHookBlock)block {
    ...
    if (argumentCount == 2) {
        imp = imp_implementationWithBlock(^id (id _self) {
            ...
            id returnValue = [TRuntimeHelper performHookedMethod:hookedSelector withArgs:@[] forObject:object isInstanceMethod:isInstanceMethod];
            if (block) {
                block(_self, @[]);
            }
        ...
        imp = imp_implementationWithBlock(^id (id _self, id arg1){
            ...
            if (arg1) {
                returnValue = [TRuntimeHelper performHookedMethod:hookedSelector withArgs:@[arg1] forObject:object isInstanceMethod:isInstanceMethod];
                if (block) {
                    block(_self, @[arg1]);
                }
            } else {
                returnValue = [TRuntimeHelper performHookedMethod:hookedSelector withArgs:@[] forObject:object isInstanceMethod:isInstanceMethod];
                if (block) {
                    block(_self, @[]);
                }
            }
        ...
        imp = imp_implementationWithBlock(^id (id _self, id arg1, id arg2){
            ...
            id returnValue = [TRuntimeHelper performHookedMethod:hookedSelector withArgs:@[arg1,arg2] forObject:object isInstanceMethod:isInstanceMethod];;
